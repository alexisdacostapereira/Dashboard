...
public class RoutingController {
    ...
    public String get() {
        String text= "Welcome on my API !\nTo use this API, here is the routes: \n /weather \n /insta \n /minecraft \n /beer\n /pokemon\n /zipcode \n /country \n /ip \n /word \n /about.JSON";
        return text;
    }
    ...
    public String getAboutJSON() throws IOException {
        String theUrl="";
         theUrl = getJSON("http://api.openweathermap.org/data/2.5/weather?q="+"Paris"+"&units=c&appid="+"27a7819b7aad62361821424c0e887f17");
         theUrl += getJSON("https://random-word-api.herokuapp.com/word?number=1");
         theUrl += getJSON("http://api.zippopotam.us/fr/78370");
         theUrl += getJSON("http://mcapi.us/server/status?ip="+"PLAY.SKYDARIA.COM"+"&port=25565");
         theUrl += getJSON("https://pokeapi.co/api/v2/pokemon/ditto");
         theUrl += getJSON("https://restcountries.eu/rest/v2/name/"+"France"+"?fullText=true");
         theUrl += getJSON("https://api.punkapi.com/v2/beers/random");
         //theUrl += getJSON("https://instagram.com/"+"tanguyxdemon"+"/?__a=1");
        return theUrl;
    }
